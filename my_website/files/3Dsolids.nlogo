...
to go
  if-else display-color = 135
  [
    set display-color 5]
  [
    set display-color display-color + 0.5
  ]
...
to calculate-turtle-position
  ifelse Rotation = "x"                                     ; sideview
  [
      set z-pos p * (sin phi) * (sin theta)
      set y-pos p * (sin phi) * (cos theta)
      set x-pos p * (cos phi)
  ]
  [
    ifelse Rotation = "y"                                  ; topview
    [
      set x-pos p * (sin phi) * (sin theta)
      set z-pos p * (sin phi) * (cos theta)
      set y-pos p * (cos phi)
    ]
    [
